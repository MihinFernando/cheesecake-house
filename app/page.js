...
const TIKTOK_URL = 'https://www.tiktok.com/@the.cheesecake.ho?_r=1&_t=ZS-97G649SALxc'
const SIZE_OPTIONS = [
...
function getProductVariants(product, productVariants = []) {
  const dbVariants = productVariants
    .filter(variant => String(variant.product_id) === String(product?.id) && variant.is_available !== false)
    .sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0))
    .map(variant => ({
      key: variant.size_key,
      label: variant.label,
      weight: variant.weight,
      price: variant.price,
    }))

  if (dbVariants.length > 0) return dbVariants
...
function ProductCard({ product, productVariants, delay, waLink }) {
  const variants = getProductVariants(product, productVariants)
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    supabase.from('product_variants').select('*').eq('is_available', true).order('sort_order')
    ...
    supabase.from('gallery_images').select('*').eq('is_visible', true).order('sort_order')
    ...
    <div className="bg-[#fcf8f7] text-[#1c1b1b] antialiased scroll-smooth">
          ...
          {products.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {visibleProducts.map((p, i) => (
                <ProductCard key={p.id} product={p} productVariants={productVariants} delay={i * 100} waLink={waLink} />
              ))}
            </div>
          )}
          {products.length > 4 && (
      ...
      {galleryImages.length > 0 && (
        <section id="gallery" className="py-24 bg-white">
          <div className="max-w-7xl mx-auto px-6 md:px-16">
            <FadeIn className="text-center mb-16">
              <h2 className="font-serif text-3xl md:text-4xl font-bold text-[#1c1b1b] mb-4">Made Fresh Gallery</h2>
              <p className="text-[#454742] max-w-xl mx-auto">A peek at the cheesecakes and bakes coming out of our kitchen.</p>
              <div className="w-16 h-1 bg-[#735c00] rounded-full mx-auto mt-6" />
            </FadeIn>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
              {galleryImages.map((image, index) => (
                <FadeIn key={image.id} delay={index * 75}>
                  <figure className="group overflow-hidden rounded-2xl bg-[#f1edec] shadow-sm">
                    <div className="relative aspect-square overflow-hidden">
                      <Image
                        src={image.image_url}
                        alt={image.alt_text || image.caption || 'The Cheesecake House gallery image'}
                        fill
                        unoptimized
                    ...
                    {image.caption && (
                      <figcaption className="px-4 py-3 text-sm font-semibold text-[#454742]">
                        {image.caption}
                      </figcaption>
                    )}
                  </figure>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-16 grid grid-cols-1 md:grid-cols-3 gap-10">
          ...
          <FadeIn delay={200} direction="left">
            ...
            <div className="flex gap-4">
              {[
                { href: 'https://www.facebook.com/profile.php?id=61554919787148&mibextid=wwXIfr&mibextid=wwXIfr', label: 'Facebook', path: 'M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z' },
                { href: 'https://www.instagram.com/the_cheesecake_house_sl?igsh=MTl0NmljanJ0MmE4MQ%3D%3D&utm_source=qr', label: 'Instagram', path: 'M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636.247 1.363.416 2.427.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z' },
                { href: TIKTOK_URL, label: 'TikTok', path: 'M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-5.2 1.74 2.89 2.89 0 012.31-4.64c.3 0 .59.05.86.13V9.4a6.33 6.33 0 00-1-.08A6.34 6.34 0 003 15.66a6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.34-6.34V8.73a8.16 8.16 0 004.77 1.52V6.82c-.29 0-.58-.04-.86-.13z' },
              ].map(({ href, label, path }) => (
                <a key={href} href={href} target="_blank" rel="noopener noreferrer"
                  aria-label={label}
                  className="text-[#454742] hover:text-[#735c00] hover:scale-125 transition-all duration-300 inline-flex h-6 w-6 items-center justify-center">
